...
void solve(int N){
  ...
  //create 1, 2, ..., N
  vi deg(2 * N - 2);
  F0R(i, sz(deg)) deg[i] = newNode();
  int negOne = newNode();
  sub(zero, one, negOne);

  for(int i = 1; i < 2 * N - 2; i += 2){
    sub(deg[i], negOne, deg[i]);
  }
  ...
  { //find root
    ...
    FOR(i, 1, N + 1) posIndex[i] = newNode(), posDeg[i] = newNode();
    ...
    for(int j = 1; j <= N; ++j){
      ...
      int cond = dum[9];     
      //cond = 1 => extraIndex = posIndex[i], posDeg[i] = A[dummy]
      conditionalCopy(extraIndex, posIndex[j], cond);
      conditionalCopy(posDeg[j], pw, cond);
    }
    print(extraIndex);
    F0R(e, N - 1){
      int uid = 2 * e, vid = 2 * e + 1;
      cmp(extraIndex, uid, dum[0]);
      mul(dum[0], one, dum[0]);
      sub(deg[vid], dum[0], deg[vid]);
    }
  }
  ...
  for(int _ = 1; _ < N; ++_){
    ...
    for(int j = 1; j < 2 * N - 2; j += 2){
      ...
      conditionalCopy(extraIndex, j, cond);
      conditionalCopy(deg[j], pw, cond);
